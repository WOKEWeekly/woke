...
const SubscriberCrud = ({ subscriber: currentSubscriber, operation, title, user }) => {
    const [stateSubscriber, setSubscriber] = useState({
      ...
      useEffect(() => {
        if (!isCreateOperation) { 
          const { createTime, firstname, lastname, email, subscriptions } = currentSubscriber;
      ...
      const buildRequest = () => {
        ...
        const subscriber = {
          ...
          email: email.trim(),
          subscriptions: subscriptions ? JSON.stringify({"articles":true}) : JSON.stringify({"articles":false}),
