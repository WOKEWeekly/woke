...
module.exports = function (app, conn, knex) {
    ...
    async.waterfall(
      [
        function (callback) {
          ...
          conn.query(SQL.SESSIONS.READ.UPCOMING, function (
            err,
            [session] = []
          ) {
      ...
      async.waterfall(
        [
          ...
          function (candidate, callback) {
            ...
            const { sql, values } = SQL.CANDIDATES.UPDATE(
              id,
              candidate,
              changed
            );
  ...
  app.put('/api/v1/topics/:id/vote/:option(yes|no)', validateReq, function (
    req,
    res
  ) {
    ...
    async.waterfall(
      [
        function (callback) {
          // Increment vote
          conn.query(SQL.TOPICS.UPDATE.VOTE(id, option), function (
            err,
            result
          ) {
            if (err) return respondToClient(res, err);
      ...
      async.waterfall(
        [
          function (callback) {
            ...
            conn.query(SQL.USERS.READ.SINGLE(), id, function (
              err,
              [user] = []
            ) {
              if (err) return callback(err);
              if (!user)
                return callback(ERROR.INVALID_ENTITY_ID(ENTITY.USER, id));
  ...
  app.put(
    '/api/v1/users/:id/clearance/:value',
  ...
  app.notify('/api/v1/users/:id/email/verify', validateReq, function (
    req,
    res
  ) {
    const { id } = req.params;
        ...
        function (user, callback) {
          ...
          jwt.sign(
            ...
            (err, token) => {
              ...
              emails(callback, [{ token }]).resendVerificationEmail(
                user,
                token
              );
            }
        ...
        function (user, callback) {
          ...
          jwt.sign(
            ...
            (err, token) => {
              if (err) return callback(err);
              if (!emailsOn) return callback(null, { token });
              emails(callback, [{ token }]).sendAccountRecoveryEmail(
                user,
                token
              );
            }
  ...
  app.put('/api/v1/pages', verifyToken(CLEARANCES.ACTIONS.EDIT_PAGE), function (
    ...
    conn.query(sql, values, function (err, result) {
      if (err) return respondToClient(res, err);
      if (result.affectedRows === 0)
        err = ERROR.INVALID_ENTITY_NAME(ENTITY.PAGE, page);
      respondToClient(res, err, 200);
    });
  });
        ...
        [
          ...
          function (document, callback) {
            ...
            query.asCallback(function (err, [id] = []) {
              ...
              if (err.errno === 1062)
                err = ERROR.DUPLICATE_ENTITY_NAME(
                  ENTITY.DOCUMENT,
                  document.name
                );
              callback(err);
