...
export class MainNavbar extends Component {
  ...
  componentDidMount(){
    const height = this.navbar.current.listNode.clientHeight;
  ...
  handleToggle = (expanding) => {
    this.setState({ navExpanding: expanding});
  }

  render(){
    const { user, theme } = this.props;
    const { imageLoaded, imageSrc, navHeight, navExpanding } = this.state;
    ...
    return (
      <Navbar
        ...
        style={{
          ...
          height: navExpanding ? 'initial' : '10vh'
        }}
        ...
        <Navbar.Toggle />
        <Navbar.Collapse>
          <Nav className="ml-auto" ref={this.navbar}>
